...
async function addVoiceoverToProject(project) {
    try {
        ...
        if (project.generation?.model === 'infinitetalk') {
            console.log(`✅ [TTS] Video generated by InfiniteTalk — audio is natively synced. Bypassing FFmpeg muxing.`);
...
router.post('/ugc-pro/qads/v2/generate-video', protect, requireCredits('qAdsGenerate'), async (req, res) => {
    try {
        ...
        // Route to InfiniteTalk if we have an avatar and an audio track
        if (avatarFaceRefs.length > 0 && audioUrl) {
            console.log(`🗣️ [Q-Ads V2] Audio + Avatar detected -> Routing to InfiniteTalk for lip-sync`);
            finalModel = 'infinitetalk';
            genResult = await submitInfiniteTalkVideoGeneration({
                prompt: finalPrompt,
                imageUrl: avatarFaceRefs[0], // InfiniteTalk takes a single portrait
                refAudio: audioUrl,
                duration,
                resolution,
            });
