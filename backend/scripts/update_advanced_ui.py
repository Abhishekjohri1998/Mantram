...
with open('frontend/src/components/VideoStudio/AdvancedMode.jsx', 'r', encoding='utf-8') as f:
    content = f.read()
...
def replacement_logic(match):
    return """<div className="vm-upper-controls">
                                ...
                                <div style={{width: '100%', marginBottom: 12, paddingBottom: 12, borderBottom: '1px dashed var(--sys-border)', display: 'flex', gap: 6, alignItems: 'center'}}>
                                        <span dangerouslySetInnerHTML={{ __html: (m.has.multishot ? shots[0].prompt : prompt).replace(/(@image\\\\d+|@video\\\\d+|@audio\\\\d+)/g, '<span style="color: var(--sys-primary)">$1</span>') }}></span>💡 {m.desc}</span>
...
with open('frontend/src/components/VideoStudio/AdvancedMode.jsx', 'w', encoding='utf-8') as f:
    f.write(content)
