...
export async function ugcPromptBuilderNode(state) {
    ...
    const userPrompt = [
        ...
        '',
        `CRITICAL LANGUAGE RULE: ALL spoken dialogue MUST be written DIRECTLY in ${settings.language || 'English'} using native script/characters. NOT English. Write as a native speaker would naturally speak.`,
        settings.language && settings.language !== 'English' ? `WARNING: If ANY dialogue line is in English instead of ${settings.language}, the ENTIRE output is REJECTED.` : '',
        `DIALOGUE FORMAT REQUIREMENT: Every shot where the avatar speaks on camera MUST include a line in this EXACT format:`,
        `DIALOGUE [emotion]: "[exact words the presenter says in ${settings.language || 'English'}]"`,
        `Emotion tags: excited, warm, urgent, calm, playful, dramatic, curious, confident, empathetic, mysterious`,
        `Write at least 4-6 distinct DIALOGUE lines across different shots. Emotions MUST arc across the video.`,
        settings.language && settings.language !== 'English' ? `Example: DIALOGUE [excited]: "[write this in ${settings.language} script/characters]"` : `Example: DIALOGUE [curious]: "Has anyone else been dealing with this or is it just me?"`,
        settings.language && settings.language !== 'English' ? `Example: DIALOGUE [warm]: "[write this in ${settings.language} script/characters]"` : `Example: DIALOGUE [excited]: "I literally wear this every event!"`,
