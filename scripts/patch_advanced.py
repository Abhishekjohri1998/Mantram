...
# Background rendering replacement
backgroundRender = """            {/* Background Grid */}
            <div className="vm-bg-grid">
                 {bgProjects.map((p, i) => (
                      <div key={p._id || i} className="vm-bg-item">
                           <video 
                               src={`API_BASE/video-studio/${p._id}/video`} 
            ...
            {/* ══════════ COMPOSE — Floating Card at Bottom ══════════ */}""".replace("API_BASE", "${API_BASE}")

content = re.sub(
    r'\{\/\* ══════════ COMPOSE — Floating Card at Bottom ══════════ \*\/\}',
    backgroundRender,
    content
)

# Replace vm-card
# NOTE: The trailing </div> is removed from newCard.
newCard = """<div className="vm-card">
                            {/* Panel Header */}
                            <div className="vm-card-header">
                                ...
                                <div style={{display: 'flex', gap: 12}}>
                                    <button style={{background: 'none', border: 'none', color: videoMode === 't2v' ? 'var(--sys-text)' : 'var(--sys-text-muted)', cursor: 'pointer', transition: '0.2s', padding: 0}} onClick={() => setVideoMode('t2v')} title="Text to Video">
                                        <span className="material-symbols-outlined" style={{fontSize: 18}}>text_fields</span>
                                    </button>
                                    <button style={{background: 'none', border: 'none', color: videoMode === 'i2v' ? 'var(--sys-text)' : 'var(--sys-text-muted)', cursor: 'pointer', transition: '0.2s', padding: 0}} onClick={() => setVideoMode('i2v')} title="Image to Video">
...
content = re.sub(r'<div className="vm-card">[\s\S]*?(?=<\/div>\s*<\/div>\s*<\/div>\s*\)\}\s*<\/>)', newCard, content)
