...
with open('scratch_advancedmode.jsx', 'r') as f:
    content = f.read()

newCss = """const css = `
/* Layout */
.vm-studio-root { position: relative; width: 100%; min-height: 100%; display: flex; flex-direction: column; }
.vm-layout { position: relative; flex: 1; display: flex; flex-direction: column; justify-content: center; align-items: center; padding: 40px 16px; overflow: hidden; }

/* Background Grid */
.vm-bg-grid { position: absolute; inset: 0; z-index: 0; display: grid; grid-template-columns: repeat(4, 1fr); gap: 4px; pointer-events: none; opacity: 0.95; padding: 4px; }
...
.vm-bg-item { width: 100%; height: 100%; object-fit: cover; border-radius: 8px; opacity: 0.4; transition: opacity .4s, transform .5s; position: relative; overflow: hidden; pointer-events: auto; }
...
.vm-bg-fade { position: absolute; inset: 0; z-index: 1; pointer-events: none; background: radial-gradient(circle at center, transparent 0%, var(--sys-surface) 100%); opacity: 0.8; }

/* Director Panel (Floating Card) */
.vm-card { width: 100%; max-width: 860px; background: var(--sys-surface-glass); border: 1px solid var(--sys-border); border-radius: 24px; padding: 0; backdrop-filter: blur(24px); box-shadow: 0 30px 60px rgba(0,0,0,0.3); z-index: 10; display: flex; flex-direction: column; color: var(--sys-text); font-family: 'Inter', sans-serif; position: relative; overflow: hidden; }
...
/* Prompt area */
.vm-prompt { padding: 20px 24px 0; position: relative; flex: 1; min-height: 120px; }
...
.vm-config-menu { position: absolute; bottom: calc(100% + 6px); left: 0; min-width: 140px; max-height: 220px; overflow-y: auto; background: var(--sys-surface-glass); backdrop-filter: blur(20px); border: 1px solid var(--sys-border); border-radius: 12px; padding: 4px; z-index: 50; box-shadow: 0 10px 30px rgba(0,0,0,0.6); }
...
.vm-generate { padding: 12px 32px; border-radius: 12px; font-weight: 700; font-size: 14px; cursor: pointer; border: none; display: flex; align-items: center; justify-content: center; gap: 8px; color: #111; background: #eab308; box-shadow: 0 4px 15px rgba(234,179,8,0.2); transition: all .2s; }
.vm-generate:hover { transform: translateY(-2px); box-shadow: 0 8px 25px rgba(234,179,8,0.4); background: #fde047; }
...
content = re.sub(r'const css = `[\s\S]*?`\s*function ConfigDropdown', newCss, content, flags=re.MULTILINE)

# Add bgProjects prop tracking
content = re.sub(
    r'export default function AdvancedMode\(\{ activeBrand, initialData \}\) \{',
    r"export default function AdvancedMode({ activeBrand, initialData, projects = [] }) {\n    const bgProjects = projects.filter(p => (p.status === 'done' || p.status === 'critique') && p.generation?.videoUrl).slice(0, 16);",
    content
)

# Wrapping root component with Background logic
oldRenderLogic = r'    return \(\s*<>\s*<style>\{css\}<\/style>'
newRenderLogic = """    return (
        <div className="vm-studio-root">
            ...
            <div className="vm-bg-fade" />
"""
content = re.sub(oldRenderLogic, newRenderLogic, content)

# Modify phase === 'compose' block to use standard layout formatting and our features combined with the reference UX
oldCardBlockRegex = r'<div className="vm-layout">[\s\S]*?(?=<\/div>\s*<\/div>\s*\)\}\s*<\/>)'

newCardBlock = """<div className="vm-layout">
                        <div className="vm-card">
                            {/* Panel Header */}
                            <div className="vm-card-header">
                                <span style={{display: 'flex', alignItems: 'center', gap: 8}}>
                                    <span className="material-symbols-outlined" style={{fontSize: 18, color: '#eab308'}}>movie_creation</span> Director Panel
                                </span>
                                <div style={{display: 'flex', gap: 12}}>
                                    <button style={{background: 'none', border: 'none', color: videoMode === 't2v' ? 'var(--sys-text)' : 'var(--sys-text-muted)', cursor: 'pointer', transition: '0.2s', padding: 0, fontWeight: videoMode==='t2v'?600:500}} onClick={() => setVideoMode('t2v')} title="Text to Video">
                                        Text to Video
                                    </button>
                                    <button style={{background: 'none', border: 'none', color: videoMode === 'i2v' ? 'var(--sys-text)' : 'var(--sys-text-muted)', cursor: 'pointer', transition: '0.2s', padding: 0, fontWeight: videoMode==='i2v'?600:500}} onClick={() => setVideoMode('i2v')} title="Image to Video">
                                        Image to Video
                                    </button>
                                </div>
                            </div>
                            ...
                            <div className="vm-upper-controls">
                                ...
                                <div style={{width: 1, height: 32, background: 'var(--sys-border)', margin: '0 8px'}}></div>
                                ...
                                <div className="vm-quality-group">
                                    ...
                                    <button className={`vm-quality-pill ${quality === 'quality' ? 'active' : ''}`} onClick={() => setQuality('quality')}><span className="material-symbols-outlined" style={{ fontSize: '14px' }}>auto_awesome</span> Pro</button>
                                </div>
                            </div>
...
content = re.sub(oldCardBlockRegex, newCardBlock, content)

# Fix outer wrap
content = re.sub(r'<\/div>\s*<\/div>\s*\)\}\s*<\/>', r'</div>\n            )}\n        </div>\n    )', content)
