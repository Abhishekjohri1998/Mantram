    ...
    return (
        <DashboardLayout title="Conversation Studio" subtitle="AI-powered inbox for Instagram & Facebook">
                        ...
                        {[
                            ...
                            <div key={i} className="glass-panel rounded-xl px-4 py-3 flex items-center gap-3 flex-1">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm text-slate-500 uppercase tracking-widest">{s.label}</p>
                                </div>
                            </div>
                        ))}
                    </div>
            ...
            <div className="flex gap-0 h-[calc(100vh-260px)] rounded-2xl overflow-hidden glass-panel">
                        ...
                        {STATUS_TABS.map(t => (
                            <button key={t.id} onClick={() => setStatusFilter(t.id)}
                                className={`flex-1 py-2 px-2 rounded-lg text-[11px] font-bold transition-all cursor-pointer flex items-center justify-center gap-1
                                    ${statusFilter === t.id ? 'bg-primary/10 text-primary' : 'text-slate-500 hover:text-white hover:bg-white/[0.04]'}`}>
                    ...
                    <div className="flex-1 overflow-y-auto">
                        {!hasBrand ? (
                            <div className="flex flex-col items-center justify-center h-full px-6 text-center">
                                ...
                                <p className="text-white font-bold mb-1">Select a Brand</p>
                                <p className="text-slate-500 text-xs">Choose a brand profile first to view and manage conversations.</p>
                            </div>
                        ) : loading ? (
                            ...
                            <div className="flex flex-col items-center justify-center h-full px-6 text-center">
                                ...
                                <p className="text-slate-500 text-xs">Instagram & Facebook DMs will appear here automatically via Meta integration.</p>
                            </div>
                        ) : threads.map(t => (
                            <button key={t._id} onClick={() => selectThread(t)}
                                className={`w-full text-left px-4 py-3.5 border-b border-white/[0.04] transition-all cursor-pointer
                                ...
                                <div className="flex items-center gap-3">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <div className="flex items-center justify-between">
                                            <p className="text-base font-bold text-white truncate">{t.contact?.name || 'Unknown'}</p>
                                        ...
                                        <div className="flex items-center gap-1.5 mt-0.5">
                                            ...
                                            <p className="text-sm text-slate-500 truncate">{t.lastMessagePreview || 'New conversation'}</p>
                                        </div>
                                    </div>
                                    {/* Unread badge */}
                                    {t.unreadCount > 0 && (
                                        <div className="size-5 rounded-full bg-primary flex items-center justify-center shrink-0">
                                            <span className="text-sm font-bold text-white">{t.unreadCount}</span>
                                        </div>
                                    )}
                                </div>
                                {/* Intent + AI badge */}
                ...
                <div className="flex-1 flex flex-col">
                    {!conversation ? (
                        <div className="flex-1 flex flex-col items-center justify-center text-center px-8">
                            ...
                            <p className="text-white font-bold text-lg mb-1">Select a conversation</p>
                        ...
                        <>
                            ...
                            <div className="p-4 border-b border-white/[0.06] flex items-center justify-between">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-sm text-slate-500">{contact?.platformUsername ? `@${contact.platformUsername}` : contact?.platform}</p>
                                    </div>
                                    {/* Channel badge */}
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    {conversation.isAIHandling && (
                                        <button onClick={handleTakeover}
                                            className="px-3 py-1.5 rounded-lg text-xs font-bold bg-amber-500/10 text-amber-400 border border-amber-500/20 hover:bg-amber-500/20 transition-all cursor-pointer">
                                            <span className="material-symbols-outlined text-sm mr-1">person</span>Take Over
                                        </button>
                                    )}

                                    <button onClick={handleResolve}
                                        className="px-3 py-1.5 rounded-lg text-xs font-bold bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 hover:bg-emerald-500/20 transition-all cursor-pointer">
                            ...
                            {compliance && (
                                <div className={`px-4 py-2 flex items-center gap-2 text-xs font-bold
                                    ${compliance.type === 'open' ? 'bg-emerald-500/5 text-emerald-400' :
                                        compliance.type === 'restricted' ? 'bg-amber-500/5 text-amber-400' :
                            ...
                            <div className="flex-1 overflow-y-auto p-4 space-y-3">
                                {conversation.messages.map((m, i) => (
                                    <div key={m._id || i} className={`flex ${m.role === 'brand' ? 'justify-end' : m.role === 'system' ? 'justify-center' : 'justify-start'}`}>
                                        {m.role === 'system' ? (
                                            <div className="px-3 py-1.5 rounded-full bg-white/[0.04] text-slate-500 text-xs font-medium">
                                                {m.content}
                                            </div>
                                        ) : (
                                            <div className={`max-w-[70%] px-4 py-3 rounded-2xl text-sm leading-relaxed ${m.role === 'brand'
                                                ...
                                                <div className="flex items-center gap-2 mt-1.5">
                                                    ...
                                                    {m.sentBy === 'ai' && m.aiConfidence && (
                                                        <span className={`text-xs font-bold px-1.5 py-0.5 rounded ${m.aiConfidence >= 80 ? 'bg-emerald-500/10 text-emerald-400' :
                                                            m.aiConfidence >= 60 ? 'bg-amber-500/10 text-amber-400' : 'bg-rose-500/10 text-rose-400'
                                                            }`}>
                                                            AI {m.aiConfidence}%
                                                        </span>
                                                    )}
                                                    {m.sentBy === 'human' && m.role === 'brand' && (
                                                        <span className="text-xs text-slate-600">👤 Human</span>
                                                    )}
                                                </div>
                                            </div>
                                        )}
                            ...
                            <div className="p-4 border-t border-white/[0.06]">
                                ...
                                {suggestions.length > 0 && (
                                    <div className="mb-3">
                                        <p className="text-sm text-slate-500 uppercase tracking-widest font-bold mb-2 flex items-center gap-1">
                                        ...
                                        <div className="flex gap-2 overflow-x-auto pb-1">
                                            {suggestions.map((s, i) => (
                                                <button key={i} onClick={() => handleReply(s.content, 'ai')}
                                                    className="shrink-0 text-left px-3 py-2 rounded-xl bg-white/[0.03] border border-white/[0.06] hover:border-primary/30 hover:bg-primary/5 transition-all cursor-pointer max-w-[200px] group">
                                                    <div className="flex items-center gap-1.5 mb-1">
                                                        <span className="text-xs font-bold text-primary">{s.label}</span>
                                                        <span className={`text-xs font-bold px-1 py-0.5 rounded ${s.confidence >= 80 ? 'bg-emerald-500/10 text-emerald-400' :
                                                            s.confidence >= 60 ? 'bg-amber-500/10 text-amber-400' : 'bg-rose-500/10 text-rose-400'
                                                            }`}>{s.confidence}%</span>
                                                    </div>
                                                    <p className="text-[11px] text-slate-400 line-clamp-2 group-hover:text-white transition-colors">{s.content}</p>
                                                </button>
                                            ))}
                                        </div>
                                    </div>
                ...
                {/* RIGHT — Contact Card */}
                <div className="w-72 border-l border-white/[0.06] overflow-y-auto">
                    {conversation && contact ? (
                        <div className="p-4 space-y-5">
                            ...
                            <div className="text-center">
                                ...
                                {contact.platformUsername && <p className="text-sm text-slate-500">@{contact.platformUsername}</p>}
                                <div className="flex items-center justify-center gap-2 mt-2">
                                    <span className={`px-2 py-1 rounded-lg text-xs font-bold ${CHANNEL_CONFIG[conversation.channel]?.bg} ${CHANNEL_CONFIG[conversation.channel]?.color}`}>
                                        {contact.platform}
                                    </span>
                                    <span className={`px-2 py-1 rounded-lg text-xs font-bold
                                        ${contact.leadStatus === 'hot' ? 'bg-rose-500/10 text-rose-400' :
                                            contact.leadStatus === 'warm' ? 'bg-amber-500/10 text-amber-400' :
                                                contact.leadStatus === 'converted' ? 'bg-emerald-500/10 text-emerald-400' :
                            ...
                            {conversation.intent && conversation.intent !== 'unknown' && (
                                <div className="glass-panel rounded-xl p-3">
                                    <p className="text-sm text-slate-500 uppercase tracking-widest font-bold mb-2">Detected Intent</p>
                                    <div className="flex items-center gap-2">
                                        <span className="material-symbols-outlined text-primary">{INTENT_ICONS[conversation.intent]}</span>
                                        <div>
                                            <p className="text-base font-bold text-white capitalize">{conversation.intent.replace(/_/g, ' ')}</p>
                                            <div className="flex items-center gap-1.5 mt-1">
                                                <div className="w-16 h-1.5 rounded-full bg-white/[0.06] overflow-hidden">
                                                    <div className={`h-full rounded-full ${conversation.intentConfidence >= 80 ? 'bg-emerald-500' :
                                                        conversation.intentConfidence >= 60 ? 'bg-amber-500' : 'bg-rose-500'
                                                        }`} style={{ width: `${conversation.intentConfidence}%` }} />
                                                </div>
                                                <span className="text-sm text-slate-500">{conversation.intentConfidence}%</span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            ...
                            {/* Contact Details */}
                            <div>
                                <p className="text-sm text-slate-500 uppercase tracking-widest font-bold mb-2">Details</p>
                                <div className="space-y-2">
                                    {[
                                        ...
                                        <div key={i} className="flex items-center gap-2 text-xs">
                                            <span className="material-symbols-outlined text-slate-600 text-sm">{d.icon}</span>
                            ...
                            {/* Tags */}
                            <div>
                                <p className="text-sm text-slate-500 uppercase tracking-widest font-bold mb-2">Tags</p>
                            ...
                            {/* Actions */}
                            <div className="space-y-2">
                                <p className="text-sm text-slate-500 uppercase tracking-widest font-bold mb-2">Actions</p>
                                <button className="w-full flex items-center gap-2 px-3 py-2 rounded-xl bg-white/[0.03] border border-white/[0.06] text-slate-400 text-xs hover:bg-primary/5 hover:text-primary hover:border-primary/20 transition-all cursor-pointer">
                        ...
                        <div className="flex flex-col items-center justify-center h-full px-4 text-center">
                            ...
                            <p className="text-xs text-slate-600">Select a conversation to view contact details</p>
                        </div>
                    )}
                </div>
            </div>
