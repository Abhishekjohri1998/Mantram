...
export default function StudioSEOBlock({ studioName, description, features, contentBlocks, faqs }) {
    ...
    return (
        <div className="mt-16 pt-16 border-t border-white/[0.04] animate-fade-in pb-12">
            ...
            <div className="max-w-5xl mx-auto px-6">
                <div className="mb-12 text-center">
                    ...
                    <h2 className="text-3xl font-black text-white mb-4">About the {studioName}</h2>
                ...
                <div className="grid md:grid-cols-2 gap-6 mb-16">
                    {contentBlocks.map((block, i) => (
                        <div key={i} className="glass-panel p-6 rounded-2xl border border-white/[0.05] hover:border-primary/20 transition-all group">
                            ...
                            <div className={`text-sm text-slate-400 leading-relaxed overflow-hidden transition-all duration-300 ${expandedBlocks[i] ? 'max-h-[1000px]' : 'max-h-24 relative'}`}>
                                {block.content}
                                {!expandedBlocks[i] && (
                            ...
                            <button 
                                ...
                                className="mt-3 text-primary text-xs font-bold hover:text-white transition-colors flex items-center gap-1"
                            >
                ...
                <div className="mb-16 glass-panel rounded-3xl p-8 border border-white/[0.05] relative overflow-hidden">
                ...
                <div className="max-w-3xl mx-auto">
                    ...
                    <div className="space-y-3">
                        {faqs.map((faq, i) => (
                            <div key={i} className="glass-panel border border-white/[0.05] rounded-xl overflow-hidden transition-all">
                                <button 
                                    onClick={() => toggleFaq(i)}
                                ...
                                >
                                    ...
                                    <span className={`material-symbols-outlined text-slate-500 transition-transform duration-300 ${expandedFaqs[i] ? 'rotate-180' : ''}`}>
                                        keyboard_arrow_down
                                    </span>
                                </button>
                                <div className={`overflow-hidden transition-all duration-300 ${expandedFaqs[i] ? 'max-h-96 opacity-100' : 'max-h-0 opacity-0'}`}>
                                    <div className="p-5 pt-0 text-sm text-slate-400 leading-relaxed border-t border-white/[0.02] mt-2">
                                        {faq.answer}
                                    </div>
                                </div>
                            </div>
