...
const STATUS_META = {
    ...
    done:       { label:'Done',       icon:'check_circle',  cls:'text-emerald-400 bg-emerald-400/10 border-emerald-400/20' },
    ...
    return (
        ...
        >
            ...
            <span className="text-[10px] text-[var(--sys-text-muted)] truncate flex-1 group-hover:text-[var(--sys-text)] transition-colors">
                {entry.sourceTitle || entry.caption?.slice(0,24) || entry.contentType || 'Item'}
            </span>
        ...
        <div className="fixed inset-0 z-[70] flex items-center justify-center p-4" onClick={onClose}>
            ...
            >
                ...
                <div className="p-5 space-y-4 max-h-[60vh] overflow-y-auto">
                    ...
                    {entry.caption && (
                        <div>
                            <p className="text-[10px] uppercase tracking-widest font-bold text-[var(--sys-text-muted)] mb-1">Caption / Description</p>
...
// MAIN COMPONENT
// ═══════════════════════════════════════════════════════════════════════════
export default function GlobalCalendarDrawer({ open, onClose }) {
    ...
    const fetchEntries = useCallback(async () => {
        if (!activeBrand?._id || !open) return
        setLoading(true)
        try {
            const data = await brandCalendar.month({ brand: activeBrand._id, month, year })
            setEntries(data.entries || [])
        } catch (err) {
            console.error('[GlobalCalendarDrawer] fetch error:', err)
        } finally {
            setLoading(false)
        }
    }, [activeBrand?._id, month, year, open])

    useEffect(() => { fetchEntries() }, [fetchEntries])

    // Trap keyboard close
    useEffect(() => {
        const handler = (e) => { if (e.key === 'Escape' && open && !selectedEntry && !schedDrawer) onClose() }
        window.addEventListener('keydown', handler)
        return () => window.removeEventListener('keydown', handler)
    }, [open, onClose, selectedEntry, schedDrawer])
    ...
    const filtered = entries.filter(e => {
        ...
        if (filterStatus   !== 'all' && e.status   !== filterStatus && e.status !== 'done')   return false
        return true
    })
    ...
    const published  = entries.filter(e => ['published', 'done', 'complete'].includes(e.status)).length
    const planned    = entries.filter(e => ['pending','in_progress','processing'].includes(e.status)).length
    ...
    return (
        <>
            ...
            <aside
                ref={drawerRef}
                className={`fixed top-0 right-0 h-full w-full max-w-4xl bg-[var(--sys-bg)] border-l border-[var(--sys-border)] z-50 flex flex-col transition-transform duration-300 shadow-2xl ${open ? 'translate-x-0' : 'translate-x-full'}`}
            >
                {/* Header */}
                <div className="flex items-center justify-between px-6 py-4 border-b border-[var(--sys-border)] shrink-0">
                    <div>
                        <h2 className="text-lg font-bold text-[var(--sys-text)] flex items-center gap-2">
                            <span className="material-symbols-outlined text-primary">calendar_month</span>
                            Global Calendar
                        </h2>
                        <p className="text-xs text-[var(--sys-text-muted)] mt-0.5">Everything scheduled or generated across all studios</p>
                    </div>
                    <button onClick={onClose} className="p-2 rounded-xl hover:bg-[var(--sys-surface)] text-[var(--sys-text-muted)] cursor-pointer transition-colors">
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-6">
                    ...
                    <div className="grid grid-cols-3 gap-4">
                        {[
                            ...
                            { label:'Published/Done',  value: published,  icon:'task_alt',     color:'text-emerald-400' },
                    ...
                    <div className="flex flex-wrap items-center gap-3 justify-between">
                        ...
                        <div className="flex items-center gap-2 flex-wrap">
                            ...
                            >
                                ...
                                {['instagram','facebook','linkedin','youtube','email','mantram'].map(p => (
                                    <option key={p} value={p} className="capitalize">{p === 'mantram' ? 'Video Generation' : p.charAt(0).toUpperCase()+p.slice(1)}</option>
                                ))}
                            </select>
                    ...
                    {loading ? (
                        ...
                        /* Month Grid */
                        <div className="bg-[var(--sys-surface)] border border-[var(--sys-border)] rounded-2xl overflow-hidden shadow-sm">
                            ...
                            <div className="grid grid-cols-7">
                                {grid.map((day, idx) => {
                                    ...
                                    return (
                                        <div key={idx}
                                            className={`min-h-[100px] p-2 border-b border-r border-[var(--sys-border)] ${!day ? 'bg-[var(--sys-bg)]/30' : 'hover:bg-[var(--sys-bg)]/50'} transition-colors`}
                        ...
                        <div className="space-y-3">
                            {filtered.length === 0 ? (
                                <div className="flex flex-col items-center justify-center py-20 text-[var(--sys-text-muted)]">
                                    ...
                                    <p className="text-sm font-medium">No items for {MONTHS[month-1]} {year}</p>
                                </div>
                                ...
                                return (
                                    <div key={entry._id}
                                        ...
                                        className="group flex items-center gap-4 p-4 bg-[var(--sys-surface)] border border-[var(--sys-border)] rounded-2xl hover:border-primary/20 cursor-pointer transition-all shadow-sm"
                                        style={{ animation: `fadeUp 0.3s ease-out ${idx*40}ms both` }}
                                    >
                                        {entry.imageUrl && (
                                            <img src={entry.imageUrl} alt="" className="w-16 h-16 rounded-xl object-cover shrink-0 border border-[var(--sys-border)]" onError={e=>e.target.style.display='none'} />
                                        )}
                                        {!entry.imageUrl && (
                                            <div className="w-16 h-16 rounded-xl shrink-0 border border-[var(--sys-border)] flex items-center justify-center bg-[var(--sys-bg)]">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            ...
                                            {entry.scheduledAt && (
                                                <p className="text-[10px] text-[var(--sys-text-muted)] mt-1 flex items-center gap-1">
                                                    <span className="material-symbols-outlined text-[11px]">schedule</span>
                                                    {new Date(entry.scheduledAt).toLocaleString('en-IN', { weekday:'short', day:'numeric', month:'short', hour:'2-digit', minute:'2-digit' })}
                                                </p>
                                            )}
                                        </div>
                                        <span className={`shrink-0 text-[11px] font-bold px-2.5 py-1 rounded-full border flex items-center gap-1 ${sm.cls}`}>
