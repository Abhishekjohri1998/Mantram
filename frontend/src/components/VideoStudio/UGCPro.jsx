...
export default function UGCPro({ activeBrand, projects = [], projectsLoaded = false, canCreateVideo = true, onUpgradeRequired, user }) {
    ...
    const handleBuildPrompt = useCallback(async () => {
        ...
        try {
            const data = await apiJson('/video-studio/ugc-pro/build-prompt', {
                ...
                settings: { style, mood, environment, hookStyle, hookShot, duration, aspectRatio, language, cta, model: selectedModel, refAudio: seedAudioUrl || null },
            })
            setPromptText(data.prompt)
            setPromptReady(true)
        } catch (err) { setError(err.message) }
        setBuildingPrompt(false)
    }, [activeBrand, productData, productImageUrls, avatarUrl, style, mood, environment, hookStyle, hookShot, duration, aspectRatio, language, cta, selectedModel, seedAudioUrl])
    ...
    const handleGenerate = useCallback(async (forcedPrompt = null) => {
        ...
        setLoading(false)
    }, [activeBrand, productData, productImageUrls, avatarUrl, style, mood, environment, hookStyle, hookShot, duration, aspectRatio, language, cta, selectedModel, canCreateVideo, onUpgradeRequired, promptText, seedAudioUrl])
            ...
            <div className="ugc-layout">
                <div className="ugc-card">
                    ...
                    {selectedModel.startsWith('seedance') && showAudioGenerator && (
                        <div style={{ margin: '0 16px 12px 16px', padding: '16px', display: 'flex', flexDirection: 'column', gap: 12, borderRadius: 10, background: 'rgba(255,255,255,0.02)', border: '1px solid var(--sys-border)', animation: 'traySlideUp 0.15s ease-out' }}>
                    ...
                    <div className="ugc-bottom">
                        <div className="ugc-bottom-left">
                            ...
                            {selectedModel.startsWith('seedance') && (
                                <button type="button" className="ugc-cfg-btn" onClick={() => setShowAudioGenerator(!showAudioGenerator)} style={{ color: seedAudioUrl ? '#a855f7' : 'inherit' }}>
                                    <span className="material-symbols-outlined" style={{ fontSize: 14 }}>music_note</span>
                                    {seedAudioUrl ? 'Seed Audio Ready' : 'Seed Audio'}
                                </button>
                            )}
                            {promptReady && (
